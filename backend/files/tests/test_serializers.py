...
class FileSerializerTests(APITestCase):
    ...
    def test_serializer_valid_data(self):
        ...
        file_obj = File.objects.create(
            file=uploaded_file, user=self.user, comment="Test comment"
        )
    ...
    def test_serializer_read_only_fields(self):
        ...
        file_obj = File.objects.create(file=uploaded_file, user=self.user)
    ...
    def test_serializer_create_file(self):
        ...
        file_data = {"file": uploaded_file, "comment": "Created via serializer"}
        ...
        # original_name теперь автоматически определяется из имени файла
    ...
    def test_serializer_update_file(self):
        ...
        file_obj = File.objects.create(
            file=uploaded_file, user=self.user, comment="Original comment"
        )
    ...
    def test_public_serializer_fields(self):
        ...
        file_obj = File.objects.create(
            file=uploaded_file, user=self.user, comment="Public file"
        )
    ...
    def test_serializer_file_size_validation(self):
        """Тест валидации размера файла (больше 100MB)"""
        # Создаем файл размером 101MB
        # Используем более простой подход - проверяем валидатор напрямую
        from files.serializers import validate_file_size
        from django.core.files.uploadedfile import SimpleUploadedFile

        # Создаем байтовый объект, который эмулирует большой файл
        class FakeLargeFile:
            def __init__(self, size):
                self.size = size

        fake_file = FakeLargeFile(101 * 1024 * 1024)  # 101MB
    ...
    def test_serializer_file_under_limit(self):
        """Тест валидации файла меньше 100MB"""
        ...
        fake_file = FakeSmallFile(50 * 1024 * 1024)  # 50MB

        # Не должно вызвать исключения
        result = validate_file_size(fake_file)
        self.assertEqual(result.size, 50 * 1024 * 1024)
