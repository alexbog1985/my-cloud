...
class FileViewSetTests(BaseTestCase):
    ...
    def test_retrieve_file_not_owner(self):
        ...
        self.client.force_authenticate(user=self.user1)
        response = self.client.get(
            reverse("files-detail", kwargs={"pk": self.file2.id})
        )
    ...
    def test_create_file_unauthenticated(self):
        ...
        response = self.client.post(
            reverse("files-list"), {"file": uploaded_file, "original_name": "test.txt"}
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_download_file_owner(self):
        ...
        self.client.force_authenticate(user=self.user1)
        response = self.client.get(
            reverse("files-download", kwargs={"pk": self.file1.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_download_file_admin(self):
        ...
        self.client.force_authenticate(user=self.admin)
        response = self.client.get(
            reverse("files-download", kwargs={"pk": self.file2.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_download_file_info_owner(self):
        ...
        self.client.force_authenticate(user=self.user1)
        response = self.client.get(
            f"{reverse('files-download', kwargs={'pk': self.file1.id})}?info=true"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class FileDownloadByLinkViewTests(BaseTestCase):
    """Тесты для FileDownloadByLinkView - скачивание по специальной ссылке"""

    def setUp(self):
        """Создание тестовых пользователей и файлов"""
        super().setUp()
        self.user1 = User.objects.create_user(
            username="user1",
            email="user1@test.com",
        ...
        self.file_content1 = b"File content for special link"
        self.file1 = File.objects.create(
            file=SimpleUploadedFile("specialfile.txt", self.file_content1),
            user=self.user1,
            comment="File with special link",
        )

        self.client = APIClient()

    def test_download_by_link_valid(self):
        """Успешное скачивание файла по специальной ссылке"""
        response = self.client.get(f"/api/s/{self.file1.special_link}/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, self.file_content1)

    def test_download_by_link_info(self):
        """Получение информации о файле по специальной ссылке"""
        response = self.client.get(f"/api/s/{self.file1.special_link}/?info=true")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["original_name"], "specialfile.txt")
        self.assertEqual(response.data["comment"], "File with special link")

    def test_download_by_link_nonexistent(self):
        """Попытка скачивания по несуществующей ссылке"""
        response = self.client.get("/api/s/nonexistent_link_12345/")
        self.assertEqual(response.status_code, 404)

    def test_download_by_link_file_deleted(self):
        """Попытка скачивания файла, которого нет на диске"""
        import os

        # Создаем файл и удаляем его с диска
        file_obj = File.objects.create(
            file=SimpleUploadedFile("tobedeleted.txt", b"test content"),
            user=self.user1,
            comment="File to be deleted",
        )
        special_link = file_obj.special_link

        # Удаляем физический файл
        if os.path.exists(file_obj.file.path):
            os.remove(file_obj.file.path)

        # Пробуем скачать через ссылку
        response = self.client.get(f"/api/s/{special_link}/")
        self.assertEqual(response.status_code, 404)
