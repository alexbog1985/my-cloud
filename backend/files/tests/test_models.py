...
class FileModelTest(BaseTestCase):
    ...
    def test_file_size_calculation(self):
        ...
        file_obj = File.objects.create(
            file=uploaded_file, user=self.user, comment="1KB file"
        )
    ...
    def test_file_string_representation(self):
        ...
        file_obj = File.objects.create(
            file=uploaded_file, user=self.user, comment="Test file"
        )
    ...
    def test_original_name_preserved_after_save(self):
        """Тест, что original_name сохраняется правильно и не перезаписывается UUID"""
        file_content = b"Test content"
        uploaded_file = SimpleUploadedFile("my_document.pdf", file_content)
        ...
        # Проверяем, что original_name - это оригинальное имя, а не UUID
        self.assertEqual(file_obj.original_name, "my_document.pdf")
        self.assertNotEqual(file_obj.original_name, file_obj.file.name)

        # Проверяем, что UUID в пути файла
        self.assertIn(".pdf", file_obj.file.name)

        # Обновляем файл и проверяем, что original_name не меняется
        file_obj.comment = "Updated comment"
        file_obj.save()
        file_obj.refresh_from_db()
